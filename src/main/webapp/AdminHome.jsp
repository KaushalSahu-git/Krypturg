 			...
 			%>
 					<div class="bg-warning p-2 m-2">
 						<p>
 							...
 							%>
 								<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Accepted&page=AdminHome" class="btn btn-sm btn-success mx-2">Accept</a>
 								<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Rejected&page=AdminHome" class="btn btn-sm btn-danger mx-2">Reject</a>
 							<%	
 							}
 							if(status.equalsIgnoreCase("Accepted")){
 	 						%>
 	 							<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Delivered&page=AdminHome" class="btn btn-sm btn-success mx-2">Delivered</a>
 	 						<%	
 	 						}
 							%>
 						</p>
